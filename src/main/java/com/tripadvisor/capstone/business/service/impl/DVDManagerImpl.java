...
import com.tripadvisor.capstone.business.dao.impl.DVDDaoImpl;
import com.tripadvisor.capstone.business.dao.impl.MovieDaoImpl;
...
import com.tripadvisor.capstone.business.exception.ComponentException;
import com.tripadvisor.capstone.business.exception.ComponentException.InvalidRequest;
import com.tripadvisor.capstone.business.service.DVDManager;
...
public class DVDManagerImpl implements DVDManager {
     ...
     * 
     * @see
     * com.tripadvisor.capstone.business.service.DVDManager#findDVDById(java
     * .util.UUID)
     */
     ...
     * 
     * @see
     * com.tripadvisor.capstone.business.service.DVDManager#findMovieById(java
     * .util.UUID)
     */
     ...
     * 
     * @see
     * com.tripadvisor.capstone.business.service.DVDManager#share(java.util.
     * List, java.util.UUID)
     */
     ...
     * 
     * @see
     * com.tripadvisor.capstone.business.service.DVDManager#borrow(com.tripadvisor
    ...
    public void borrow(Borrow borrow) throws ComponentException {
        ...
        List<DVD> availableDVDs = dvdDao.findAvailableDVDs(borrow.getDvdTitle());
        if(CollectionUtils.isNotEmpty(availableDVDs)) {
            //sort by dvds add to the system to give the first lender chance to rent his title first 
            Collections.sort(availableDVDs, new DVDAddOnComparator());
            //remove the first DVD 
            DVD dvd = availableDVDs.get(0);
            //check again that DVD is not null and status to make sure threads have not overwritten the status
            if(dvd!=null && dvd.getStatus().equals(Status.AVAILABLE.getValue())) {
                ...
                //when all the updates to the table are successful send email to the lender
                //Note : Hard coding the email as we dont have user table - out of scope of this project
                emailManager.send("temp@gmail.com", "You DVD is been rented ", " You DVD "+dvd.getTitle()+" is been rented");
            }
        } else {
     ...
     * 
     * @see
     * com.tripadvisor.capstone.business.service.DVDManager#search(java.util
     * .Map)
     */
